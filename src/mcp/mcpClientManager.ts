...
// McpConfig as SdkMcpConfig is not used, so removed. If needed, it would likely be from @modelcontextprotocol/sdk/types.js
import Database from 'better-sqlite3';
import { MCPConfig } from './types.js';
import { FunctionDeclaration } from '@google/genai/node'; // Type is not directly used here, FunctionDeclaration is.
...
export class McpClientManager {
  ...
  private loadUserConfigs(): void {
     ...
     for (const row of configs) {
         ...
         // Store the config, client will be initialized on demand
    ...
    for (const [serverName, serverEntry] of userClients.entries()) {
      try {
        ...
        if (!serverEntry.client) {
            console.warn(`Client for server "${serverName}" for user ${userId} is not connected. Skipping tools.`);
            continue;
        }

        // Get tools from the client instance
         const mcpTools = serverEntry.client!.listToolsSync(); // Non-null assertion after connectClientForUser and check
         if (!mcpTools || !mcpTools.tools) continue;

        // Map MCP Tool objects to Gemini FunctionDeclaration format
        const geminiServerTools = mcpTools.tools.map(tool => mapMcpToolToGeminiFunctionDeclaration(tool, serverName));
  ...
  async callTool(chatId: number, toolCall: any): Promise<any> { // toolCall format from Gemini, maybe refine type
    ...
    if (!serverEntry.client) {
        throw new Error(`Client for server "${serverName}" for chat ${chatId} is not connected. Cannot call tool.`);
    }
   ...
   private async connectClientForUser(userId: number, serverName: string, config: MCPConfig): Promise<Client> {
       ...
       // The isConnected() method might not exist on the MCP SDK Client, rely on client presence
       if (serverEntry.client) {
       ...
       if (serverEntry.client) {
           console.log(`Client for "${serverName}" for user ${userId} found but not connected. Attempting reconnect/new.`);
       ...
       try {
           const client = new Client({
               name: `feathers-studio-bot-client-user${userId}`, // Unique name per user
               version: '1.0.0',
           }, {
               // ClientCapabilities type is inferred by TypeScript from the Client constructor options
               capabilities: {
                   roots: { enabled: true }, // Assuming roots support is desired
                   sampling: {} // Assuming sampling support is desired with default options
               }
           });
           ...
           if (config.type === 'stdio') {
               ...
               transport = new StdioClientTransport({
                ...
                // TODO: Implement HTTP client transport
                // console.warn('HTTP transport type is not fully supported in this example. Only Stdio.');
                // For now, let's assume StreamableHTTPClientTransport is the correct one if it exists
                // If this class doesn't exist or is not intended for client-side, this will fail.
    ...
    for (const [userId, userClients] of this.activeServers.entries()) {
        ...
        for (const [serverName, serverEntry] of userClients.entries()) {
            if (serverEntry.client && serverEntry.client.close) {
                ...
                serverEntry.client = null; // Mark as null after initiating close
            }
        }
  ...
  private setupClientListeners(userId: number, client: Client): void {
    ...
    // The listeners should update the state for the specific user's clients
     ...
     client.on('error', (error) => {
         ...
         const userClients = this.activeServers.get(userId);
         if (userClients && userClients.has(serverName)) {
             userClients.get(serverName)!.client = null as any; // Mark as disconnected
         }
     });
